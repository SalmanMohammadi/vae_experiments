...
plt.figure()
plt.imshow(sample[0][0].view(64, 64), cmap='Greys')
mu, _, mu_prime, _ = model.encode(sample[0][0].unsqueeze(dim=0).to(DEVICE))
...
# another sample
plt.figure()
plt.imshow(sample[0][1].view(64, 64), cmap='Greys')
mu, _, mu_prime, _ = model.encode(sample[0][1].unsqueeze(dim=0).to(DEVICE))
z = torch.cat((mu, mu_prime), dim=1).detach().cpu()
num_samples = 11
results = []
for dimension in np.arange(config_.hparams['z_size']):
...
plt.figure()
plt.imshow(sample[0][2].view(64, 64), cmap='Greys')
mu, _, mu_prime, _ = model.encode(sample[0][2].unsqueeze(dim=0).to(DEVICE))
z = torch.cat((mu, mu_prime), dim=1).detach().cpu()
num_samples = 11
results = []
for dimension in np.arange(config_.hparams['z_size']):
